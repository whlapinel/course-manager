...
func (svc *MarkdownService) MarkdownToHTML(srcPath string) error {
...
func (svc *MarkdownService) RenderMarkdownFile(path string) ([]byte, error) {
	...
	if err := svc.Markdown.Convert(contents, &buf); err != nil {
...
func (svc *MarkdownService) renderMarkdownFiles(title, filesPath string) error {
	...
	for _, entry := range mdFiles {
		...
		markdownGroup.Go(func() error {
			err := svc.renderMarkdown(entry, title, inputPath, filesPath)
			if err != nil {
				return err
			}
			return nil
...
func (svc *MarkdownService) renderMarkdown(entry os.DirEntry, title, inputPath, filesPath string) error {
