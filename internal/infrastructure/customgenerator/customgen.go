...
type customGenerator struct {
	dataPaths       ports.PathingService
	staticSitePaths ports.PathingService
}
...
func New(
	...
	return &customGenerator{
		dataPaths:       dataPaths,
		staticSitePaths: staticSitePaths,
	}
}
...
func StaticSiteRootDir(user dto.User) string {
...
func (svc *customGenerator) generate(user dto.User, term dto.Term) error {
	...
	err = CopyNodeDir(svc.dataPaths.NodeDirPath(nodes.ToSlice()...), svc.staticSitePaths.NodeDirPath(nodes.ToSlice()...))
	if err != nil {
		return err
	}
...
func renderLessonPages(nodes ports.Nodes, errChan chan<- error, wg *sync.WaitGroup, ctx context.Context, cancel context.CancelFunc) error {
	...
	default:
		...
		assFrag := AssessmentsFragment(lessonpage, nodes.Lesson.(dto.Lesson).Assessments, nodes)
		wg.Add(1)
